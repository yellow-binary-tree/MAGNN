...
class SLTester(TestPipLine):
    def __init__(self, model, hps, exp, test_dir=None, limited=False, blocking_win=3):
    ...
    def evaluation(self, inputs, combined_extractable_nodes, combined_labels, index, dataset, blocking=False, compute_loss=True):
        """
            :param G: the model
        ...
        """
        self.batch_number += 1
        combined_logits, _ = self.model.forward(inputs, combined_extractable_nodes)

        if compute_loss:
            loss = self.criterion(combined_logits, combined_labels).unsqueeze(-1)    # [n_nodes, 1]
            loss = loss.mean()
        ...
        for i in range(self.hps.batch_size):
            ...
            if self.m == 0:
                ...
                if blocking:
                    pred_idx = self.ngram_blocking(original_article_sents, p_sent[:, 1], self.blocking_win, min(self.m, sent_number))
                else:
                    # print(p_sent.size())
                    topk, pred_idx = torch.topk(p_sent[:, 1], min(self.m, sent_number))
                    pred_idx, _ = torch.sort(pred_idx)
                prediction = torch.zeros(sent_number).long()
                prediction[pred_idx] = 1
            self.extracts.append(pred_idx.tolist())
            self.pred += prediction.sum()

            if compute_loss:
                label = combined_labels[sent_start_number:sent_start_number+sent_number].cpu()
                self.true += label.sum()
            ...
            self.example_num += 1
            hyps = "\n".join(original_article_sents[id] for id in pred_idx if id < sent_number)

            self._hyps.append(hyps)
            self._refer.append(refer)
        return self.extracts[-len(index):], self._hyps[-len(index):], self._refer[-len(index):]     # for logging output
...
def eval_label(match_true, pred, true, total, match):
    match_true, pred, true, match = float(match_true), float(pred), float(true), float(match)
